...
pwsh scripts/smoke-msix.ps1   # AM-22 sideload harness: the built .msix is unsigned (Partner Center re-signs),
                              # so this mints a self-signed cert whose Subject matches the package's own
                              # Publisher, signs a COPY, installs it, then prints the Store smoke checklist.
                              # -DryRun resolves and reports without changing anything; -Cleanup uninstalls
                              # and removes the cert — run it before installing the real Store build, since
                              # both share the package identity and cannot coexist.
